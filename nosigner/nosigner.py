...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sec-file",
                        help="Read the secret key (nsec or 64-hex) from a file "
                             "(use a 0600 file; preferred over --sec on the cmdline)")
...
def main() -> None:
    ...
    # --sec-file takes precedence over --sec so the key never appears in argv/ps.
    raw_sec = args.sec
    if getattr(args, "sec_file", None):
        try:
            raw_sec = Path(args.sec_file).read_text().strip()
        except OSError as e:
            logger.error(f"Could not read --sec-file: {e}")
            sys.exit(1)

    if raw_sec:
        if raw_sec.startswith("nsec1"):
            privkey = decode_nsec(raw_sec)
        elif len(raw_sec) == 64:
            privkey = bytes.fromhex(raw_sec)
        else:
